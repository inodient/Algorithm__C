...
int ReHash(char* String, int Start, int Size, int HashPrev,
		int Coefficient) {
	if (Start == 0) {
		return HashPrev;
	}
...
int Test_KarpRabin(int argc, char** argv) {
	char* FilePath;
	FILE* fp;
	...
	while (fgets(Text, MAX_BUFFER, fp) != NULL) {
		int Position = KarpRabin(Text, strlen(Text), 0, Pattern,
				PatternSize);
